    ...
    for root, dirs, files in os.walk(INPUT_DIR):
        for file in files:
            if file.lower().endswith(VALID_EXTENSIONS):
                ...
                # 计算相对路径，以保持 train/val/OK/NG 这种目录树结构不乱
                ...
                if img is not None:
                    ...
                    # 每处理 500 张打印一次进度，防止你看终端以为卡死了
                    if processed_count % 500 == 0:
                        print(f"⏳ 已处理 {processed_count} 张图片...")

    print("-" * 50)
    print(f"✅ 完美竣工！共计为 {processed_count} 张图片添加了工业级黑边。")
    print(f"📁 请使用新文件夹进行训练: {OUTPUT_DIR}")
